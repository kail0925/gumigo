...
def check_collision(dx, dy):
    ...
    if MAP[int(player_y / TILE_SIZE)][int(hit_box_x / TILE_SIZE)] == 0:
        player_x = next_x
    next_y = player_y + dy
    hit_box_y = next_y + (margin if dy > 0 else -margin)
    if MAP[int(hit_box_y / TILE_SIZE)][int(player_x / TILE_SIZE)] == 0:
        player_y = next_y
...
def start_reload():
    ...
    if current_weapon == WEAPON_PISTOL and current_pistol_ammo < max_pistol_ammo:
        ...
        is_reloading = True
        reload_end = pygame.time.get_ticks() + 3000
...
def shoot():
    ...
    wall_dist = Z_BUFFER[CASTED_RAYS // 2]
    ...
    if target_enemy:
        ...
        target_x = player_x + math.cos(player_angle) * (wall_dist - 2)
        target_y = player_y + math.sin(player_angle) * (wall_dist - 2)
        wall_h_at_shot = (TILE_SIZE * current_proj_dist) / max(1.0, wall_dist)
        hit_ratio = (-player_pitch) / wall_h_at_shot if wall_h_at_shot > 0 else 0
...
def draw_ui():
    ...
    virtual_screen.blit(PROG_FONT.render(f"체력: {int(max(0, player_health))}", True, WHITE), (20, HEIGHT - 80))
    ...
    if is_reloading:
        ...
        if current_weapon == WEAPON_PISTOL:
            ammo_text = PROG_FONT.render(f"[권총] {current_pistol_ammo} / ∞", True, WHITE)
        else:
            ammo_text = PROG_FONT.render(f"[AK-47] {current_ak_ammo} / {ak_reserve_ammo}", True, WHITE)
    virtual_screen.blit(ammo_text, (WIDTH - 280, HEIGHT - 40)) 
    ...
    if is_aiming:
        if current_weapon == WEAPON_PISTOL:
            ...
            pygame.draw.line(virtual_screen, RED, (WIDTH//2 - 35, HEIGHT//2), (WIDTH//2 + 35, HEIGHT//2), 1)
            pygame.draw.line(virtual_screen, RED, (WIDTH//2, HEIGHT//2 - 35), (WIDTH//2, HEIGHT//2 + 35), 1)
    else:
    ...
    if heal_effect_timer > 0:
        ...
        heal_msg = PROG_FONT.render(heal_message, True, GREEN)
        heal_msg.set_alpha(alpha)
...
while True:
        ...
        if game_state == STATE_GAMEOVER:
            ...
            if game_state == STATE_PLAYING and not player_dead:
                ...
                if sens_minus_btn_rect.collidepoint(virtual_mouse_pos): sens_mult = max(0.1, sens_mult - 0.1)
                    ...
                    game_state = STATE_PLAYING; pygame.mouse.set_visible(False); pygame.event.set_grab(True); pygame.mouse.get_rel()
                elif restart_btn_rect.collidepoint(virtual_mouse_pos):
                    init_game(); pygame.mouse.set_visible(False); pygame.event.set_grab(True); pygame.mouse.get_rel()
                elif quit_btn_rect.collidepoint(virtual_mouse_pos): pygame.quit(); exit()
            ...
            if game_state == STATE_PLAYING:
                ...
                if event.key == pygame.K_1 and current_weapon != WEAPON_PISTOL: current_weapon = WEAPON_PISTOL; is_reloading = False
                if event.key == pygame.K_2 and has_ak and current_weapon != WEAPON_AK: current_weapon = WEAPON_AK; is_reloading = False
    ...
    if game_state == STATE_PLAYING:
        ...
        target_max_enemies = min(40, 15 + (current_score // 500) * 2) 
            ...
            for item in items[:]:
                ...
                if dist < 40: 
                    if item['type'] == 'ak':
                        ...
                        ak_reserve_ammo += 30; heal_effect_timer = 40
                    elif item['type'] == 'ammo':
                        ak_reserve_ammo += 10; heal_message = "총알 획득 (+10발)"; heal_effect_timer = 40
                    elif item['type'] == 'health':
                        player_health = min(100.0, player_health + 25); heal_message = "체력 회복!"; heal_effect_timer = 40
                    items.remove(item)
